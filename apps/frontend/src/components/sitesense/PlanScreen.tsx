...
import { getFeatureConfig } from "@/lib/sitesense/featureTags";
...
export function PlanScreen({ plan, onContinue, onBack }: Props) {
  const icon = getFeatureConfig(plan.detected.feature)?.icon ?? "📍";

  return (
    <div
      className="ss-grid-bg min-h-screen px-6 py-12 max-w-2xl mx-auto"
      style={{ color: TEXT }}
    >
      {/* Header */}
      <div className="mb-8">
      ...
      <div className="flex gap-3">
        <button
          ...
          style={{ borderColor: P[3], color: TEXT_MUTED, background: "transparent" }}
        >
          <Edit2 size={14} />
          Edit prompt
        </button>
